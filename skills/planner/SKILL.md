...
5. 让每个新增文件、接口、依赖和验证都能对应当前目标、明确演进压力或必要交付基础设施；
   未能对应的内容留到以后，不预置未知未来。
6. 新的 Assurance v4 代码任务必须冻结可独立完成的 `work_units`：每项声明负责人、目标、依赖、
   精确范围和机械完成观察。工作单元开始后不可由 Driver 临时拆分或新增；目标变化走显式计划修订。
...
- 新 v4 任务默认使用有界工作单元执行：上下文从计划、ledger、Git 和已确认 evidence 重建，不能
  把完整历史 transcript 当作进度事实。缺少工作单元或完成观察时，计划必须停止并补齐，不得退回
  整段长 turn。
