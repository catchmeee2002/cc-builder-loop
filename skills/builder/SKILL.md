...
3. `parallel_ready=false` 时，从 frozen `test_context.public_prerequisites` 读取公开前置产物，
   ...
   若 ledger 的 `interface_input_paths` 非空，initial brief 必须逐项传入对应 publication blob，并要求
   Tester 在构造 fixture 或断言前先验证；它们是 wire shape、exit/status/code、字段层级和生命周期场景
   的唯一来源，不得从错误文本、候选 diff 或实现细节猜测，也不得要求 Builder 增加兼容字段。
...
5. 成功时最后一行输出 `BUILDER_RESULT: pass run_id=<run_id>`；已按用户决定放弃时输出
   ...
   若成功 run 的 `status/finalize` 返回 `continuation.ready=true`，先输出独立行
   `BUILDER_CONTINUATION_READY:<run_id>`；仅允许同 session 紧邻 Plan mode 续接，不代表授权或自动启动新 run。
