...
| `test-run-pass-cmd-args.sh` | V2.0/V7.3: run-pass-cmd.sh 三参签名 + FATAL 契约（25 case：三参 LOG_ROOT 决定日志归档 / 两参 缺省 LOG_ROOT=RUN_CWD / FAIL 消息含 LOG_ROOT 路径 / RUN_CWD 内 loop.yml 缺失 fallback 主仓 + stderr 警告 / #67 回归：log_root==run_cwd 且无 loop.yml → FATAL 且绝不 PASS、传主仓 → 真跑过 / pass_cmd 为空 → FATAL / yaml 损坏 → FATAL） | reviewer hint 补测 |
