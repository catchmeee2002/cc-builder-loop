...
harness_init "全新仓库 builder-loop 全流程"

SCRIPTS_DIR="${HARNESS_REPO_ROOT}/skills/builder-loop/scripts"
...
section "Step 2: 一键 loop-init + 覆盖 pass_cmd"
(cd "$TMP" && bash "$SCRIPTS_DIR/loop-init.sh" "$TMP" 2>&1 | tail -5)
...
worktree:
  enabled: false
YMLEOF
git -C "$TMP" add -A && git -C "$TMP" -c core.hooksPath=/dev/null commit -m "chore(test): [cr_id_skip] Override loop.yml for E2E" --allow-empty-message >/dev/null 2>&1 || true
...
section "Step 3: setup-builder-loop"
...
RESULT="$(bash "$SCRIPTS_DIR/run-pass-cmd.sh" "$TMP" 1 2>/dev/null || true)"
LAST="$(echo "$RESULT" | tail -1)"
assert "PASS_CMD 返回 FAIL" "echo '$LAST' | grep -q 'FAIL'"

# ---- Step 5: 修复语法错误 → PASS ----
section "Step 5: 修复语法 + 再跑 PASS_CMD"
cat > "$TMP/src/main.py" <<'PYEOF'
def hello():
    return "hello"
PYEOF

RESULT2="$(bash "$SCRIPTS_DIR/run-pass-cmd.sh" "$TMP" 2 2>/dev/null || true)"
LAST2="$(echo "$RESULT2" | tail -1)"
assert "修复后 PASS_CMD 返回 PASS" "echo '$LAST2' | grep -q 'PASS'"

# ---- Step 6: 验证 trace ----
section "Step 6: 验证 trace 文件"
...
section "Step 7: stop hook 路径查找"
# 模拟 stop hook 接收到 CWD=$TMP 的 stdin
HOOK_OUT="$(echo "{\"cwd\":\"$TMP\"}" | bash "$HARNESS_HOOK" 2>&1; echo "EXIT:$?")"
