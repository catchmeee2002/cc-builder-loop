...
harness_init "V2.1 env file 加载机制"
...
# ============================================================
section "A1: 主 env 干净 + judge-env.sh 存在"
...
assert "A1 输出含 sk-666 前缀" "echo '$OUT_A1' | grep -q 'sk-666-fro'"

# ============================================================
# A2: 主 env 已设 + judge-env.sh 含别的 → 不覆盖
# ============================================================
section "A2: 主 env 已设 sk-real + 文件含 sk-666 → 不覆盖"
EC_A2=0
...
# ============================================================
# A3: 主 env 干净 + 文件不存在 → missing credentials
# ============================================================
section "A3: 主 env 干净 + 文件不存在 → exit 1"
...
# ============================================================
section "A5: loop.yml 指定别处 credentials_file"
PROJ="$TMP/proj-a5"
mkdir -p "$PROJ/.claude"
...
# ============================================================
# A6: credentials_file == 默认全局路径 → phase 1 跳过
# ============================================================
section "A6: loop.yml.credentials_file 等于默认全局路径 → 跳过 phase 1"
PROJ_A6="$TMP/proj-a6"
...
assert "A6 stderr 不含 ERROR" \
  "! echo '$STDERR_A6' | grep -qi '^ERROR:'"
assert "A6 输出中 credential_path=env" \
