...
事实都在 `.claude/builder-loop/runs/<run_id>/ledger.json`，唯一写入者是 `bl`。tester / reviewer 的结论由 SubagentStop hook 从 `last_assistant_message` 的 `BUILDER_LOOP_RESULT:` 行解析后写入，只认 SubagentStart 登记过的 agent_id。runtime 的内部提交不跑目标仓库的 git hooks；只有 `finalize --run-commit-hook` 那一次交付提交会跑。
