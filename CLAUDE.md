...
- **文档新鲜度校验用交叉比对，不用自证**：检查文档（CLAUDE.md / plan.md / SKILL.md 等）是否需要更新时，禁止"builder 自行声称已检查没问题"。做法：机械提取文档中的状态声明行 → 与本次 changed_files 交叉比对 → 逐条给结论（"仍成立" / "需更新"）。不读全文，只提取含状态语义的行
