...
- design-philosophy 原则零（机器判据 ground truth 锚点）在 reviewer 这层得到实物落地（C 件事，方向 3 轻量版）。
...
4. **frame-3**：swap 警报误判（误以为推渐进路径是 swap 用户原话"问题不需要动自然消失"——校准后澄清"问题消失"恰是 A/B/C 架构改造的副产品，跟渐进路径不冲突）。
...
具体每件事的级别：
- A 来源身份层：L3（hook 接口 + lock 文件 schema 变）
- StructuredOutput（P0-1）：L3（4 套 agent prompt 契约 + 校验重试层是新接口）
- B step 3.5 机械化：L2（builder.md prompt 改 + 机械检测清单是逻辑变化无新接口）
- C LSP/mypy 轻量：L3（PASS_CMD schema 加新 stage 类型）
- P0-2 pipeline：L2（loop 流程改造，不涉及对外接口）
- P0-3 resume journal：L3（state 加 journal 段 + 启动 restore 是新接口）
- P0-4 budget 感知：L2（max_iterations 改动态读取）
...
| **2 · 拆 designer/implementer/verifier 三 agent** | builder 拆三 agent | 不采纳激进版。**StructuredOutput（P0-1）= 温和版**——4 套 agent 输出契约化是同源方向 |
| **3 · 判据轴展开（多轴判据基建）** | 框架级判据轴清单（自洽 / 指称 / 覆盖 / 约束） | 不采纳完整版。**C 件事 = 轻量版**——只接 LSP/mypy 子集填补指称轴空白 |
...
| 代号 | 名称 | 主要消化 TODO 类别 |
|------|------|---------------------|
| ~~**A**~~ | ~~subagent 来源身份层~~ | ✅ 落地（2026-06-14） |
| **P0-1 / SO** | StructuredOutput 全套契约化 | ⏸ 搁置——blocked on CC Agent tool schema support。DEVIATION_FROM_SPEC 协议单独拆出 |
| ~~**B**~~ | ~~step 3.5 机械化检测~~ | ✅ 落地（2026-06-14） |

**V4.0（关键路径后段 + 工程化收益）**：

| 代号 | 名称 | 主要消化 TODO 类别 |
|------|------|---------------------|
| **C** | LSP/mypy 轻量接入 | reviewer 无符号语义 / 大 diff 审查深度不足类（≥6 条） |
| P0-2 | pipeline 无 barrier | 工程化收益（无直接 TODO 消化） |
| P0-3 | resume journal | 替代 V3.3 orphan 检测（部分迁移收益） |
| P0-4 | budget 感知 | 工程化收益（max_iterations 动态化） |
...
- **设计阶段可并行**：A 实施期间可启动 SO prompt 设计 + B 机械检测清单设计。
- **集成阶段串行**：SO 落地集成必须等 A 完成；C 落地集成必须等 SO 完成。
- **B / P0-2 / P0-3 / P0-4 可随时插入**：与关键路径无强依赖。

V3.5 内部目标周期 4-6 周；V4.0 同。总周期 ≈ 2-3 个月（按一周两晚 dogfooding 节奏）。

### 6.3 三类同根因升级理由（细化）

**A · 来源身份层**——`improvements.md` 至少 9 条同根因（2026-06-02 ×3 / 05-31 / 05-29 / 05-28 / 05-26 / 05-10 / 05-09），共同特征 = subagent / hook / session 之间缺"来源身份"判定层。cc-loop-tracking §3.2 已立项但状态是"等 CC 补 `agent_transcript_path`"——**等天上掉肉**。本期走 fallback 路径 A：`agent_type` 反向白名单 + lock 加 `agent_id`，不阻塞等接口。

**SO · StructuredOutput 全套契约化**——P0 之 P0，三重原则命中（一/四/六）。落地后 4 套 agent（reviewer / judge / doc-maintainer / tester）输出从"自由文本 + 正则解析"升级为"schema 校验 + 自动重试"。顺带消化 [A2] DEVIATION_FROM_SPEC 协议 / [A2] schema 字段变更兼容 / 部分 reviewer 长 diff 误读类。

**B · step 3.5 机械化**——至少 4 条同根因（05-28 / 05-24 / 05-13 / 05-11），共同特征 = "靠 builder 主观裁量"。改输入条件不改输出约束（原则四）——把 doc 评估 / commit-before-merge / plan.md 更新 等场景做成机械触发清单（pattern match + 检测规则），消除主观判断点。

**C · LSP/mypy 轻量接入**——至少 6 条同根因（06-03 / 05-31 / 05-26 ×2 / 05-10 / 05-09），共同特征 = reviewer 看 diff 没做符号语义层判断（方法名存在性 / re-export 完整性 / 长 diff 审查深度）。**承认 reviewer 在域穿越 / 指称漂移上结构性盲**（metacog C10：自洽探测器盲于保持自洽的错误），不靠它，靠正交仪器——LSP/mypy 子集填补指称轴空白，作为 PASS_CMD 新 stage 类型嵌入。这是 design-philosophy 原则零（"为自动化系统提供 ground truth 锚点"）在 reviewer 这层的实物落地。
...
| 风险 | 影响 | 应对 |
|------|------|------|
| A 阻塞 SO 启动 → V3.5 周期延长 | 高 | 设计阶段允许并行启动 SO prompt 设计；集成串行 |
| SO 改 4 套 agent prompt 耦合面大、改一处撞他处 | 高 | 4 套 agent 各自的 schema + 校验重试层独立，但用同一套基础设施（公共 schema 定义 + 公共校验工具）；先做 reviewer 一套跑通 fixture 再扩 |
| C 多语言适配跟接入项目栈差异（generator/BOT/EDB 各不同）| 中 | V4.0 起步只支持 Python（mypy）+ TypeScript（tsserver）；其他语言项目按需后续接入；项目侧 loop.yml 加可选字段 `referential_stage` |
| pipeline 改造跟 V3.0 phase 状态机交互复杂 | 中 | 设计先于实现（原则六）——写出 pipeline + phase 状态机交互的状态图作为 PR 前置条件 |

### 7.2 跨件事的风险

**StructuredOutput 跨 dotfiles 同步**：
- 4 套 agent 中 `reviewer.md` / `doc-maintainer.md` 同时被 cc-builder-loop 仓和 dotfiles（`~/.claude/agents/`）维护。SO 改造必须先确认 dotfiles 那侧是否同步——按 CLAUDE.md §3"与 dotfiles 的依赖关系"段处理。
- 应对：SO 落地前同步 dotfiles 仓的对应 agent 文件；写 sync-checklist 加一条"agent prompt schema 改造时双仓同步"。

**A 落地后 5+ 条同根因消化的对账风险**：
- A 落地后必须显式标记 improvements.md 对应条目状态（promoted to 架构改造 + V3.5 关闭），否则下一轮翻牌还会看到。
- 应对：A 完成时一次性扫 improvements.md grep 关键词（"subagent 落点 / hook 撞 session / worktree 写错"）逐条标记。SO/B/C 完成时同样扫一次。
...
- `skills/builder-loop/docs/cc-loop-tracking.md` §3.2（路径 A 状态从"待 CC 补接口"转"已落地 fallback"）
- 新增 fixture：`skills/builder-loop/fixtures/test-subagent-source-identity.sh`

### 8.2 SO（StructuredOutput）触及
...
### 8.5 P0-2 / P0-3 / P0-4 触及

- P0-2 pipeline：`scripts/builder-loop-stop.sh`（核心循环改造）+ `scripts/extract-error.sh` + 状态文件 schema 加段
- P0-3 resume journal：`scripts/locate-state.sh`（journal 加载）+ state schema 加 `journal` 段 + `scripts/setup-builder-loop.sh`（启动时 restore 逻辑）
- P0-4 budget 感知：`scripts/run-pass-cmd.sh`（max_iterations 动态读取）+ judge agent 联动
...
6. 标记 `improvements.md` 9+ 条对应同根因为 "promoted to 架构改造 V3.5/A"

**SO · StructuredOutput**（M-L 量级）：
...
**C · LSP/mypy 轻量接入**——支持 Python（mypy）和 TypeScript（tsserver）；loop.yml 加可选 `referential_stage`；fixture 双语言覆盖。

**P0-2 pipeline 无 barrier**——先写 pipeline + V3.0 phase 状态机的状态交互图作为前置条件；改造 builder-loop-stop.sh 核心循环。

**P0-3 resume journal**——state 加 journal 段；setup 时检测 journal 优先级高于 V3.3 orphan 检测；老 state 缺 journal 段时降级走旧 orphan 路径。

**P0-4 budget 感知**——loop.yml `max_iterations` 改可读 `budget.remaining()` 表达式；judge agent 联动；缺省值兜底。
...
机械化（CI 可跑）：
- A/SO/B 三件事各自 fixture 全 PASS
- `improvements.md` 至少 16 条同根因条目（A 9+ / SO 3+ / B 4+）被标记 "promoted to 架构改造 V3.5"
- V3.0/V3.3/V3.4 既有 fixture 全套不退化（兼容性 fixture 100% PASS）

人工兜底（不可机械化）：
- A 完成后跨 session 并发跑 ≥2 个 builder-loop 7 天无 lock 撞车日志
- SO 完成后 reviewer 误读 / schema 漂移类问题 4 周内零新增 improvements.md 条目
- B 完成后步骤 3.5 主观裁量 skip 类问题 4 周内零新增

### 10.2 V4.0 验收

机械化：
- C/P0-2/P0-3/P0-4 各自 fixture 全 PASS
- `improvements.md` C 类同根因（6+ 条）被标记 "promoted to 架构改造 V4.0"
- V3.5 既有 fixture 不退化

人工兜底：
- C 完成后 reviewer 符号语义类问题 8 周内零新增
- P0-2 pipeline 改造后跨任务并行的 wall-clock 实测改善（具体数字交付时落 CHANGELOG）
- P0-3 resume journal 在 ≥2 次模拟崩溃恢复 e2e 测试 PASS
...
- 当前 fallback 方案（agent_type 反向白名单 / SO 4 套 / 机械检测 / LSP 子集）是否真正消除根因
...
- fixture 类型分两类：
  - **e2e fixture**（真实跑 setup → loop → PASS → cleanup 全流程）—— A / P0-2 / P0-3
  - **unit fixture**（单脚本/单 schema 黑盒）—— SO / B / C / P0-4
...
本期把 C 件事当方向 3 轻量版做了，但完整版没做。**完整版要不要做的前提**是承认 builder-loop 是 metacog 框架的工程化载体之一这个身份——前面对话用户未接也未拒，留着。
