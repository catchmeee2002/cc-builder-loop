...
Worktree 隔离的是 Git 候选写入，不是所有文件读取和进程执行。交付代码、测试与 tracked 文档属于
candidate；宿主工具、缓存或配置只能作为只读依赖并绑定真实执行身份；共享服务、设备和唯一运行态
必须通过独立授权、probe 与可恢复事务使用。不能复制的输入不靠强行复制解决，也不能因候选外读取
本身就阻断 Agent；只有未授权写入、身份不明的验收输入或无法恢复的共享副作用才需要停止。
